...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="b0af8-103">Usein kysytyt kysymykset käynnistyksestä yritystietoja käyttäen</span><span class="sxs-lookup"><span data-stu-id="b0af8-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]


## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="b0af8-104">Miksi tällainen käynnistys on tarpeellista?</span><span class="sxs-lookup"><span data-stu-id="b0af8-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="b0af8-105">Sinulla on ehkä aiemmin luotu Common Data Servicen tai muunDynamics 365 -sovelluksen esiintymä, jossa on yritystietoja, ja haluat ottaa siihen liittyvän kaksoiskirjoitusyhteyden käyttöön.</span><span class="sxs-lookup"><span data-stu-id="b0af8-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="b0af8-106">Siinä tapauksessa Common Data Service tai muu Dynamics 365 -sovellus on käynnistettävä yritystietoja käyttäen ennen kaksoiskirjoitusyhteyden ottamista käyttöön.</span><span class="sxs-lookup"><span data-stu-id="b0af8-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="b0af8-107">Milloin tällaista käynnistystä kannattaa käyttää?</span><span class="sxs-lookup"><span data-stu-id="b0af8-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="b0af8-108">Käynnistystä yritystietoja käyttäen kannattaa käyttää ennen entiteettikarttojen kaksoiskirjoituksen käyttöönottoa (vaiheessa 5).</span><span class="sxs-lookup"><span data-stu-id="b0af8-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="b0af8-109">Määritä Finance and Operations -sovelluksen ja Common Data Servicen tai muun Dynamics 365 -sovelluksen esiintymien välinen kaksoiskirjoitusyhteys kirjautumalla Finance and Operations -sovellukseen järjestelmänvalvojana.</span><span class="sxs-lookup"><span data-stu-id="b0af8-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="b0af8-110">Siirry **Tietojen hallinta** -moduuliin napsauta **Kaksoiskirjoitus** -painiketta.</span><span class="sxs-lookup"><span data-stu-id="b0af8-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="b0af8-111">**Tietojen integrointiohjelma** käynnistyy.</span><span class="sxs-lookup"><span data-stu-id="b0af8-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="b0af8-112">Luo kaksoiskirjoitusyhteys vähintään yhdelle yritykselle.</span><span class="sxs-lookup"><span data-stu-id="b0af8-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="b0af8-113">![Kaksoiskirjoitusyhteyden luonti](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="b0af8-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="b0af8-114">Ota **Cdm_companies** -entiteettikartta käyttöön.</span><span class="sxs-lookup"><span data-stu-id="b0af8-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="b0af8-115">Näin Finance and Operations -sovelluksen yritykset synkronoidaan Common Data Serviceen.</span><span class="sxs-lookup"><span data-stu-id="b0af8-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="b0af8-116">![Entiteettikartan ottaminen käyttöön](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="b0af8-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="b0af8-117">Suorita käynnistyksen näytekoodi Common Data Servicen tai muun Dynamics 365 -sovelluksen esiintymässä.</span><span class="sxs-lookup"><span data-stu-id="b0af8-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="b0af8-118">Kun käynnistys on tehty ja järjestelmä on valmis reaaliaikaiseen synkronointiin, ota entiteettikartat käyttöön.</span><span class="sxs-lookup"><span data-stu-id="b0af8-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="b0af8-119">Entiteettikarttojen ottaminen käyttöön käynnistää käyttöönotettujen entiteettikarttojen ensimmäisen tietojen synkronoinnin.</span><span class="sxs-lookup"><span data-stu-id="b0af8-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="b0af8-120">Kaksoiskirjoitusyhteydessä valittuja yrityksiä vastaavat tiedot synkronoidaan Finance and Operations -sovelluksen ja Common Data Servicen välillä.</span><span class="sxs-lookup"><span data-stu-id="b0af8-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="b0af8-121">Miten koodinäytettä käytetään?</span><span class="sxs-lookup"><span data-stu-id="b0af8-121">How to I use the code sample?</span></span>
<span data-ttu-id="b0af8-122">Näytekoodi on C#-sovellus, jonka voi ladata Visual Studiossa.</span><span class="sxs-lookup"><span data-stu-id="b0af8-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="b0af8-123">Se käyttää NuGet-paketin riippuvuuksia Common Data Servicen SDK:ssa, ja niitä voi päivittää Visual Studio -vakiotyökaluilla.</span><span class="sxs-lookup"><span data-stu-id="b0af8-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="b0af8-124">Kun ratkaisu on purettu ja avattu Visual Studiossa ja NuGet-paketit on palautettu, etsi koodista merkkijono **TODO**.</span><span class="sxs-lookup"><span data-stu-id="b0af8-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="b0af8-125">**TODO** kirjaa jokaisen päätöksen, jonka teet käynnistämisestä yritystietoja käyttäen, ja käytössä on näytekoodia kanonista toteutusta varten.</span><span class="sxs-lookup"><span data-stu-id="b0af8-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO** , with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="b0af8-126">Näytekoodi näyttää vain yhden monista tavoista, jolla voit luokitella entiteettitietueita yrityksen mukaan.</span><span class="sxs-lookup"><span data-stu-id="b0af8-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="b0af8-127">**TODO** -osien logiikkaa muuttamalla voit luoda mukautetun luokittelun.</span><span class="sxs-lookup"><span data-stu-id="b0af8-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="b0af8-128">Mitä on odotettavissa?</span><span class="sxs-lookup"><span data-stu-id="b0af8-128">What should I expect?</span></span>
<span data-ttu-id="b0af8-129">Oletusarvoisesti näytesovellus antaa toimittaa liiketoimintayksiköstä yritykseen tapahtuvien koodien yhdistämismääritysten hakemiston.</span><span class="sxs-lookup"><span data-stu-id="b0af8-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="b0af8-130">Kaikki entiteetit, jotka käynnistetään käyttäen **OwningBusinessUnit** -kenttää, määritetään automaattisesti käyttämään määritettyä yritystä.</span><span class="sxs-lookup"><span data-stu-id="b0af8-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="b0af8-131">Kaikki entiteetit, joissa ei ole **OwningBusinessUnit** -kenttää, kuten tuote, määrittää yrityksen yhdistämismäärityksen perusteella käyttäen tyhjää liiketoimintayksikön arvoa.</span><span class="sxs-lookup"><span data-stu-id="b0af8-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="b0af8-132">Konsolisovellus odottaa yhtä parametria, joka on joko **–simulate** tai **–apply**.</span><span class="sxs-lookup"><span data-stu-id="b0af8-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="b0af8-133">Jos käytät **–simulate** komentoriviparametria, tietoja ei päivitetä.</span><span class="sxs-lookup"><span data-stu-id="b0af8-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="b0af8-134">Vain **simulation_<entityname>.csv** -tiedostot luodaan samassa hakemistossa kuin työkalu. Näitä tietoja on yksi kullekin entiteetille, joka olisi päivitetty.</span><span class="sxs-lookup"><span data-stu-id="b0af8-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="b0af8-135">Voit tarkistaa nämä tiedostot iteratiivisesti käsittelyn aikana ja varmistaa, että koodi päivittää yrityksen arvot odotetusti.</span><span class="sxs-lookup"><span data-stu-id="b0af8-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="b0af8-136">Kun simuloidut päivitykset ovat valmiit, käytä **–apply** -parametria.</span><span class="sxs-lookup"><span data-stu-id="b0af8-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="b0af8-137">Se päivittää 1 000 tietueen erissä (oletusarvo) kaikki tietueet, joissa on tällä hetkellä virheellinen yrityksen arvo.</span><span class="sxs-lookup"><span data-stu-id="b0af8-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="b0af8-138">Koodi on ainutkertainen sellaisenaan eli se voidaan suorittaa uudelleen ja vain virheellisesti määritetyt yritykset päivitetään.</span><span class="sxs-lookup"><span data-stu-id="b0af8-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="b0af8-139">Kun **–apply** suoritetaan, koodi tuottaa muutokset sisältäviä CSV-tiedostoja, joiden nimenä on **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="b0af8-139">When running with **–apply** , the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
