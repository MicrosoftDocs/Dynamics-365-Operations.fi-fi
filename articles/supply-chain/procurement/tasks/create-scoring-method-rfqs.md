---
title: Luo tarjouspyyntöjen pisteytystapa
description: Tässä menettelyssä näytetään, miten pisteytysmenetelmä luodaan.
author: RichardLuan
manager: tfehr
ms.date: 08/29/2018
ms.topic: business-process
ms.prod: ''
ms.service: dynamics-ax-applications
ms.technology: ''
ms.search.form: PurchRFQScoringMethod
audience: Application User
ms.reviewer: kamaybac
ms.search.region: Global
ms.author: riluan
ms.search.validFrom: 2016-06-30
ms.dyn365.ops.version: Version 7.0.0
ms.openlocfilehash: 738768a6756db83a6855756ef48fffb4a5874b4a
ms.sourcegitcommit: deac22ba5377a912d93fe408c5ae875706378c2d
ms.translationtype: HT
ms.contentlocale: fi-FI
ms.lasthandoff: 01/16/2021
ms.locfileid: "5021376"
---
# <a name="create-a-scoring-method-for-rfqs"></a>Luo tarjouspyyntöjen pisteytystapa

[!include [banner](../../includes/banner.md)]

Tässä menettelyssä näytetään, miten pisteytysmenetelmä luodaan. Pisteytysmenetelmä on joukko ehtoja, jonka avulla voit verrata tarjouksia, jotka lähetetään vastauksena tarjouspyyntöön. Voit esimerkiksi haluta arvioida toimittajan aiemman suoritustason perusteella tai arvioida, onko yritys ympäristöystävällinen tai hyvä yhteistyökumppani, tai voit vertailla tarjouksia hinnan perusteella. Pisteytysmenetelmän voi liittää pyyntötyyppiin oletusmenetelmäksi määrätyn tyyppisille tarjouspyynnöille. Yleensä ostopäällikkö tekee nämä tehtävät. Voit käyttää tätä menettelyä USMF-demoyrityksen tiedoilla tai omilla tiedoillasi.

1. Siirry kohtaan Hankinta > Asetukset > Tarjouspyyntö > Pisteytysmenetelmä.
2. Valitse Uusi.
3. Kirjoita arvo Nimi-kenttään.
4. Kirjoita arvo Kuvaus-kenttään.
5. Valitse Tallenna.
6. Valitse Uusi.
7. Kirjoita arvo Nimi-kenttään.
8. Kirjoita arvo Kuvaus-kenttään.
    * Kuvaus sekä pisteytysmenetelmän nimi näytetään, kun tarjouspyynnölle valitaan menetelmä.  
9. Syötä Alue alkaa -kenttään numero.
    * Väli rajoittaa, mitä hankinnan ammattilainen voi kirjata pistearvoksi. Kun tarjouspyynnöllä on useita pisteytysehtoja, annetut pistearvot lisätään toisiinsa ja summa on käytettävissä tarjousten vertailussa.  
10. Syötä Alue loppuu -kenttään numero.
11. Valitse Uusi.
12. Kirjoita arvo Nimi-kenttään.
13. Kirjoita arvo Kuvaus-kenttään.
14. Syötä Alue alkaa -kenttään numero.
15. Syötä Alue loppuu -kenttään numero.



[!INCLUDE[footer-include](../../../includes/footer-banner.md)]