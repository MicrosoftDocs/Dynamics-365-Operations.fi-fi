...
Tuoteversiot luodaan yleensä tietyille versioille, kun tuote kehittyy elinkaarensa aikana. Tuoteversiot käsitellään yksityiskohtaisesti myöhemmin tässä artikkelissa.
...
Seuraavat alueet eivät tue versiodimensioita. (Näitä alueita voidaan edelleen käyttää, mutta versioituja tuotteita (tuotteita, joissa versiodimensiota käytetään) ei voi lisätä niihin.) Versioitua nimikettä ei voi esimerkiksi lisätä toimittajan tuoteluetteloon. Tämä johtuu siitä, että versiodimension sisältämien tuotteiden lisääminen näille alueille voi aiheuttaa tärkeitä muutoksia.
