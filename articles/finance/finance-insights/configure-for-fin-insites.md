...
## <a name="configure-common-data-service"></a>Määritä Common Data Service
...
1. Avaa [Power Platformin hallintakeskus](https://admin.powerplatform.microsoft.com/) ja luo uusi Common Data Service -ympäristö samaan Active Directory -vuokraajaan seuraavasti:
    ...
    11. Kirjoita Common Data Service -organisaation tunnus muistiin.
    ...
    16. Määritä **Esiintymän viitetietojen tunnus** -kentän arvoksi sen Common Data Service -organisaation tunnuksen arvo, jonka merkitsit muistiin aiemmin.
    17. Kirjoita selaimen osoitepalkkiin Common Data Service -organisaation URL-osoite. URL-osoite voi olla esimerkiksi `https://org42b2b3d3.crm.dynamics.com`.
...
### <a name="enter-the-common-data-service-directory-id-and-the-users-azure-ad-object-id"></a>Syötä Common Data Service -hakemiston tunnus ja käyttäjän Azure AD -objektin tunnus

1. Syötä Common Data Service -hakemiston tunnus seuraavasti:

    1. Avaa [Azure-portaali](https://portal.azure.com).
    2. Kirjaudu sisään käyttämällä käyttäjätunnusta, jota käytettiin Common Data Service -ympäristön luomisessa.
...
> [!NOTE]
> Varmista, että luot seuraavat resurssit samassa Azure AD -esiintymässä kuin Common Data Service -ympäristön. Et voi käyttää eri Azure AD -esiintymän resursseja.
...
4. Syötä **Hae tiedot** -apuohjelman tietojen sivulla seuraavat arvot.
    ...
    | Arvo                                                    | kuvaus |
    |----------------------------------------------------------|-------------|
    | CDS-organisaation URL-osoite                                     | Common Data Service -esiintymän Common Data Service -organisaation URL-osoite. Jos haluat löytää tämän arvon, avaa [Power Apps -portaali](https://make.powerapps.com) ja valitse oikeassa yläkulmassa oleva **Asetukset**-painike (ratassymboli). Valitse **Lisäasetukset** ja kopioi URL-osoite. (URL-osoitteen lopussa on merkkijono dynamics.com.) |
    | CDS-organisaation tunnus                                               | Common Data Service -esiintymän ympäristön tunnus. Jos haluat etsiä tämän arvon, avaa [Power Apps -portaali](https://make.powerapps.com) ja valitse oikeassa yläkulmassa oleva **Asetukset**-painike (ratassymboli) . Valitse **Mukautukset \> Kehittäjän resurssit \> Esiintymän viitetiedot** ja kopioi **Tunnus**-kohdan arvo. |
    | CDS-vuokraajan tunnus (AAD:n hakemistotunnus)               | Common Data Service -esiintymän vuokraajan tunnus. Jos haluat etsiä tämän arvon, avaa [Azure-portaali](https://portal.azure.com), siirry **Azure Active Directory** -kohtaan ja kopioi **Vuokraajan tunnus** -kentän arvo. |
    | Anna sen käyttäjäobjektin tunnus, jolla on järjestelmänvalvojan rooli | Common Data Service -käyttäjän Azure AD -käyttäjäobjektin tunnus. Tämän käyttäjän on oltava Common Data Service -esiintymän järjestelmänvalvoja. Jos haluat etsiä tämän arvon, avaa [Azure-portaali](https://portal.azure.com) ja siirry kohtaan **Azure Active Directory \> Käyttäjät**. Valitse käyttäjä ja kopioi **Tunnistetiedot**-osassa **Objektin tunnus** -kohdan arvo. |
    | Onko tämä CDS:n oletusympäristö vuokralaiselle?      | Jos Common Data Service -esiintymä oli ensimmäinen luotu tuotantoesiintymä, valitse tämä valintaruutu. Jos Common Data Service -esiintymä on luotu manuaalisesti, poista tämän valintaruudun valinta. |
