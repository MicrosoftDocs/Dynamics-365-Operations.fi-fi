...
<span data-ttu-id="5d5a6-105">Recency, Frequency, and Monetary (RFM)-analyysi on työkalu, jonka avulla organisaatio voi arvioida asiakasostojen muodostamat tiedot.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-105">Recency, frequency, and monetary (RFM) analysis is a marketing tool that your organization can use to evaluate the data that is generated by customer purchases.</span></span> <span data-ttu-id="5d5a6-106">Kun olet määrittänyt RFM-analyysin, asiakkaille määritetään laskettu RFM-pistemäärä heidän tehdessään ostoksia.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-106">After you set up RFM analysis, customers are assigned a calculated RFM score as they make purchases.</span></span> <span data-ttu-id="5d5a6-107">RFM-pisteytys voi olla kolmenumeroinen luokitus tai yhteenlaskettu numero, riippuen siitä, kuinka organisaatio on määrittänyt RFM-analyysin.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-107">The RFM score can be a three-digit rating or an aggregate number, depending on how your organization has configured RFM analysis.</span></span> <span data-ttu-id="5d5a6-108">Luokitus toimii seuraavasti, jos organisaatio käyttää pisteytykseen kolminumeroista luokitusta:</span><span class="sxs-lookup"><span data-stu-id="5d5a6-108">Here’s how the rating works if your organization uses a three-digit rating for the score:</span></span>

- <span data-ttu-id="5d5a6-109">Ensimmäinen numero on asiakkaan recency-luokitus, joka ilmaisee kuinka äskettäin asiakas teki oston organisaatiosta.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-109">The first digit is the customer's recency rating, which is how recently the customer made a purchase from your organization.</span></span> 
- <span data-ttu-id="5d5a6-110">Toinen numero on asiakkaan frequency-luokitus, joka ilmaisee kuinka usein asiakas tekee ostoja organisaatiosta.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-110">The second digit is the customer's frequency rating, which is how often the customer makes purchases from your organization.</span></span> 
- <span data-ttu-id="5d5a6-111">Kolmas numero on asiakkaan monetary-luokitus, joka ilmaisee paljonko rahaa asiakas käyttää tehdessään ostoja organisaatiosta.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-111">The third digit is the customer's monetary rating, which is how much the customer spends when he makes purchases from your organization.</span></span> 

<span data-ttu-id="5d5a6-112">Organisaatiosi on esimerkiksi asettanut luokitukset asteikolle 1-5, jossa 5 on korkein luokitus.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-112">For example, your organization has set the ratings on a scale of 1 through 5, where 5 is the highest rating.</span></span> <span data-ttu-id="5d5a6-113">Tässä tapauksessa asiakkaan luokitus 535 kertoo seuraavaa tietoa asiakkaasta:</span><span class="sxs-lookup"><span data-stu-id="5d5a6-113">In this case, a customer rating of 535 tells you the following information about the customer:</span></span>

-   <span data-ttu-id="5d5a6-114">**Äskettäisyysluokitus 5** – Asiakas on tehnyt ostoksen äskettäin.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-114">**Recency rating of 5** – The customer recently made a purchase.</span></span>
-   <span data-ttu-id="5d5a6-115">**Toistuvuusluokitus 3** – Asiakas ostaa organisaatiosi tuotteita normaalilla aikavälillä.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-115">**Frequency rating of 3** – The customer purchases products from your organization moderately often.</span></span>
-   <span data-ttu-id="5d5a6-116">**Rahasummaluokitus 5** – Kun asiakas suorittaa oston, hän käyttää huomattavan summan rahaa.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-116">**Monetary rating of 5** – When the customer makes a purchase, he spends a significant amount of money.</span></span>

<span data-ttu-id="5d5a6-117">Jos organisaatiosi käyttää tuloksena yhteenlaskettua lukua, yksittäiset luokitusluvut lasketaan yhteen.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-117">If your organization uses an aggregate number for the score, the individual ratings are added together.</span></span> <span data-ttu-id="5d5a6-118">Saman esimerkin asiakkaalla on luokitus 13 (5 + 3 + 5).</span><span class="sxs-lookup"><span data-stu-id="5d5a6-118">For the same example, the customer has a rating of 13 (5 + 3 + 5).</span></span>

## <a name="to-set-up-rfm-analysis-for-the-customers-in-your-organization"></a><span data-ttu-id="5d5a6-119">RFM-analyysin määrittäminen organisaation asiakkaille</span><span class="sxs-lookup"><span data-stu-id="5d5a6-119">To set up RFM analysis for the customers in your organization.</span></span>

1.  <span data-ttu-id="5d5a6-120">Valitse **Puhelinkeskus** > **Kausittainen** > **RFM-analyysi**.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-120">Go to **Call center** > **Periodic** > **RFM analysis**.</span></span>

2.  <span data-ttu-id="5d5a6-121">Valitse **RFM-analyysi**-sivulla **Uusi**.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-121">On **RFM analysis** page, select **New**.</span></span> <span data-ttu-id="5d5a6-122">Anna RFM-määrityksen nimi **RFM-määritys**-kentässä.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-122">In the **RFM definition** field, enter a name for the RFM definition.</span></span> <span data-ttu-id="5d5a6-123">Esimerkiksi voisit kutsua määritelmää RFM-A:ksi</span><span class="sxs-lookup"><span data-stu-id="5d5a6-123">For example, you could call the definition RFM-A.</span></span>

3.  <span data-ttu-id="5d5a6-124">Kirjoita alkamis- ja päättymispäivä tälle RFM-määritykselle.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-124">Enter a start date and end date for this RFM definition.</span></span>

4.  <span data-ttu-id="5d5a6-125">Toimi **Yleiset**-pikavälilehdellä seuraavasti:</span><span class="sxs-lookup"><span data-stu-id="5d5a6-125">On the **General** FastTab, do the following:</span></span> 
...
1.  <span data-ttu-id="5d5a6-156">Valitse **Puhelinkeskus** > **Kirjauskansiot** > **Asiakaspalvelu**.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-156">Go to **Call center** > **Journals** > **Customer service**.</span></span> 

2.  <span data-ttu-id="5d5a6-157">Valitse **Asiakaspalvelu**-sivun **Asiakaspalvelu**-ruudun hakukentissä haettava avainsanatyyppi ja kirjoita etsittävä teksti.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-157">On the **Customer service** page, in the **Customer service** pane, in the search fields, select the keyword type to search on and enter the search text.</span></span>

3.  <span data-ttu-id="5d5a6-158">Valitse **Haku**.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-158">Select **Search**.</span></span>

4.  <span data-ttu-id="5d5a6-159">Valitse **Asiakashaku**-sivulla ensin asiakastietua ja sitten **Valitse asiakas**.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-159">On the **Customer search** page, select the customer record that you want, and then click **Select customer**.</span></span> 

<span data-ttu-id="5d5a6-160">RFM-pistemäärä näkyy **Tilaushistoria**-ryhmässä **Asiakaspalvelu**-sivun oikealla puolella.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-160">The RFM score is displayed in the **Order history** group on the right side of the **Customer service** page.</span></span> 

## <a name="view-or-clear-the-history-of-an-rfm-analysis-record"></a><span data-ttu-id="5d5a6-161">Tarkastele tai poista RFM-analyysitietueen historiatietoja.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-161">View or clear the history of an RFM analysis record</span></span>
<span data-ttu-id="5d5a6-162">Tämän toiminnon avulla voit tarkastella tai poistaa RFM-analyysitietueen historiatiedot.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-162">Use this procedure to view or clear the history of an RFM analysis record.</span></span> 

1.  <span data-ttu-id="5d5a6-163">Valitse **Puhelinkeskus** > **Kausittainen** > **RFM-analyysi**.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-163">Go to **Call center** > **Periodic** > **RFM analysis**.</span></span>

2.  <span data-ttu-id="5d5a6-164">Valitse **RFM-analyysi**-sivulla tarkasteltava tietue.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-164">On the **RFM analysis** page, select the record that you want to view.</span></span>

3.  <span data-ttu-id="5d5a6-165">Voit tarkastella tietueen historiatiedot valitsemalla **Historia**-pikavälilehden.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-165">To view the record history, select the **History** FastTab.</span></span>

4.  <span data-ttu-id="5d5a6-166">Voit tyhjentää tietueen historiatiedot valitsemalla **Tyhjennä historia**.</span><span class="sxs-lookup"><span data-stu-id="5d5a6-166">To clear the history of the record, select **Clear history**.</span></span>
